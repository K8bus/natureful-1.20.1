...
public class SawmillScreen extends HandledScreen<SawmillScreenHandler> {
    ...
    protected void drawMouseoverTooltip(DrawContext context, int x, int y) {
        ...
        if (this.canCraft) {
            ...
            List<SawingRecipe> list = ((SawmillScreenHandler)this.handler).getAvailableRecipes();

            for(int l = this.scrollOffset; l < k && l < ((SawmillScreenHandler)this.handler).getAvailableRecipeCount(); ++l) {
                int m = l - this.scrollOffset;
                int n = i + m % 4 * 16;
                int o = j + m / 4 * 18 + 2;
                if (x >= n && x < n + 16 && y >= o && y < o + 18) {
                    context.drawItemTooltip(this.textRenderer, ((SawingRecipe)list.get(l)).getOutput(this.client.world.getRegistryManager()), x, y);
                }
            }
        }
    }
    ...
    private void renderRecipeBackground(DrawContext context, int mouseX, int mouseY, int x, int y, int scrollOffset) {
        for(int i = this.scrollOffset; i < scrollOffset && i < ((SawmillScreenHandler)this.handler).getAvailableRecipeCount(); ++i) {
            ...
            if (i == ((SawmillScreenHandler)this.handler).getSelectedRecipe()) {
                n += 18;
    ...
    private void renderRecipeIcons(DrawContext context, int x, int y, int scrollOffset) {
        ...
        for(int i = this.scrollOffset; i < scrollOffset && i < ((SawmillScreenHandler)this.handler).getAvailableRecipeCount(); ++i) {
            ...
            int l = j / 4;
            int m = y + l * 18 + 2;
            context.drawItem(((SawingRecipe)list.get(i)).getOutput(this.client.world.getRegistryManager()), k, m);
        }
    }
    ...
    private boolean shouldScroll() {
        return this.canCraft && ((SawmillScreenHandler)this.handler).getAvailableRecipeCount() > 12;
    }

    protected int getMaxScroll() {
        return (((SawmillScreenHandler)this.handler).getAvailableRecipeCount() + 4 - 1) / 4 - 3;
    }
